  ...
  return (
    <Svg width={width} height={height}>
      ...
      {cells.map((c, i) => {
        ...
        return (
          <G key={i}>
            <Rect x={c.x} y={c.yTop} width={c.w} height={barH} rx={3} fill={color} opacity={isTotal ? 1 : 0.9} />
            <SvgText x={c.x + c.w / 2} y={c.type === 'neg' ? c.yBot + 13 : c.yTop - 7} textAnchor="middle" fontSize={10.5} fill={isTotal ? valColor : color} fontWeight="700">
              {valLabel}
            </SvgText>
            <SvgText x={c.x + c.w / 2} y={padding.top + h + 32} textAnchor="middle" fontSize={9.5} fill={labelColor}>
              {c.label}
            </SvgText>
          </G>
        );
