...
export default function TransactionDetailScreen({ navigation, route }: Props) {
  ...
  useLayoutEffect(() => {
    navigation.setOptions({ title: '거래 상세' });
  }, [navigation]);
      ...
      >
        <View style={styles.sectionPad}>
          <View style={[styles.summaryCard, { backgroundColor: theme.card, borderColor: theme.border }]}>
            ...
            {(tx.costType || tx.autoGenerated) && (
              <View style={styles.badgeRow}>
                {tx.costType && (
                  <View style={[styles.badge, { backgroundColor: theme.bg }]}>
                    <Text style={{ color: theme.textMuted, fontSize: 11, fontWeight: '700' }}>{tx.costType === 'FIXED' ? '고정비' : '변동비'}</Text>
                  </View>
                )}
                {tx.autoGenerated && (
                  <View style={[styles.badge, { backgroundColor: theme.brandSoft }]}>
                    <Text style={{ color: theme.brand, fontSize: 11, fontWeight: '700' }}>정기 자동생성</Text>
                  </View>
                )}
              </View>
            )}
      ...
      {!isViewer && (
        <View style={[styles.ctaWrap, { backgroundColor: theme.card, borderTopColor: theme.border }]}>
          <Pressable
            ...
            onPress={() => navigation.navigate('TransactionEdit', { mode: 'edit', txId: String(tx.id), returnTo: 'TransactionDetail' })}
          >
            <Text style={{ color: '#fff', fontSize: 14.5, fontWeight: '800' }}>수정</Text>
          </Pressable>
          <Pressable style={[styles.ctaBtn, styles.ctaBtnDanger, { borderColor: theme.danger }]} onPress={() => setConfirmDelete(true)}>
            <Text style={{ color: theme.danger, fontSize: 14.5, fontWeight: '800' }}>삭제</Text>
          </Pressable>
        </View>
      )}
      <ConfirmDialog
...
const styles = StyleSheet.create({
  ...
  ctaWrap: { flexDirection: 'row', gap: 10, paddingHorizontal: 20, paddingTop: 12, paddingBottom: 20, borderTopWidth: 1 },
  ctaBtn: { flex: 1, borderRadius: 12, paddingVertical: 14, alignItems: 'center' },
  ctaBtnDanger: { backgroundColor: 'transparent', borderWidth: 1 },
});
