...
### 엔티티
- [ ] `ad.assets` — Asset entity (현금성/투자/가상자산/실물자산/연금·보험/부채, isLiability)
- [ ] `ad.asset_snapshots` — AssetSnapshot entity (date, value, fxRateToKRW, valueKRW, INDEX)
- [ ] `ad.transactions` — Transaction entity (type, amount, categoryId, fromAssetId, toAssetId, tags, attachments jsonb, recurringTemplateId, autoGenerated)
- [ ] `ad.recurring_transactions` — RecurringTransaction entity (MONTHLY/YEARLY, dayOfMonth, monthOfYear, startDate, endDate, active, lastRunDate)

### 모듈
- [ ] `modules/assets/` — GET list, POST search/create/update/archive/delete, GET :id
- [ ] `modules/asset-snapshots/` — POST 단건 upsert, POST batch, POST delete
- [ ] `modules/transactions/` — GET recent, POST search/create/update/delete/upload-attachment
- [ ] `modules/recurring-transactions/` — GET list, POST create/update/toggle/delete/run-now
- [ ] `modules/recurring-transactions/recurring-transactions.cron.ts` — `@Cron('0 1 * * *', timeZone: 'Asia/Seoul')`
- [ ] cron 멱등성: `lastRunDate` 체크 + try-catch (부분 unique 인덱스는 init.sql raw SQL로)

### 시드 보강
- [ ] `scripts/seed/seed-ad.ts` — `seedDemoUserAndHousehold()` 추가 (couple30 페르소나: 자산 10/스냅샷 60mo/거래 80/정기 8)

### 검증
- [ ] `npm run ad:seed` 실행 후 DB에 데이터 확인
- [ ] `POST /api/ad/recurring/:id/run-now` — 거래 자동 생성 확인
