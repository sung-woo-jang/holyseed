...
**시간창**: cron 03:55+04:55(KST, 화~토) 이중 등록 = **마감 65분 전** (2026-07-17 프로브로 검증 — 소수점 금액주문 당일 체결 컷오프가 마감 40~65분 전 사이. 65분 전은 즉시 체결, 40분 전은 다음 개장 이월). `checkWindow`가 마감 60~75분 전 창 검증 (서머타임 자동 대응). 시각·창은 `LAOFUS_RUN_CRON_1/2`·`LAOFUS_WINDOW_MIN/MAX` env (ecosystem.local.config.js).
