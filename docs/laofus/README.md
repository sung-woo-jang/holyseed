...
⚠️ **소수점 주문은 당일 체결되지 않는다** (2026-07-16 첫 실주문에서 확인): `orderAmount` 시장가 주문을 장중에 넣어도 토스가 **다음 세션 개장 배치로 이월** (`orderedAt`이 다음 개장 시각으로 재설정, status PENDING 유지). 따라서 실제 흐름은:
1. KST 03:55(마감 65분 전) — 판단 + 주문 접수 → 즉시 체결 (컷오프 이전이므로 당일 체결). `laofus.pending_orders`에 원장 기록
2. 당일 22:30/23:30(다음 세션 개장) — 토스가 개장가로 체결
3. 22:40/23:40 회수 크론(+ 다음 새벽 실행 시작 시) — 체결 확인 후 trade·state DB 반영 (`reconcile`)
4. 미회수 주문이 남아 있으면 신규 주문 스킵 (중복 매수 방지). 취소/거부면 FAILED 처리, 부분 체결분은 반영
