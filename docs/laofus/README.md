...
- **2026-07-28~**: 하락장 장기화 대응으로 판단·매매 주기를 매 거래일 → **2거래일에 1회**로 완화 (원금 소진 페이스를 늦추는 목적, 쿼터매도 포함 판단 공식 자체는 무변경). `engine_state.last_decision_us_date`(직전 판단일)와 시장 캘린더의 `previousBusinessDay`를 비교해 직전 거래일에 이미 판단했으면 스킵 (`engine.service.ts`의 `run()`). 즉시 재판단이 필요하면 `force` 옵션으로 게이트 우회
