...
**Holyseed Backend** - 멀티 프로젝트 아키텍처를 지원하는 NestJS 기반 백엔드 서버입니다.
...
- 데이터베이스 기본값: localhost:5432, postgres/password123, holyseed_dev
...
- pgAdmin: http://localhost:5050 (admin@holyseed.com / admin123)
...
```bash
# PostgreSQL 상태 확인
docker exec -it holyseed_postgres_dev pg_isready -U postgres

# 데이터베이스 접속 테스트
docker exec -it holyseed_postgres_dev psql -U postgres -d holyseed_dev
```
