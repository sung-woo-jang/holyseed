 ...
 *
 * 회수 크론은 America/New_York 타임존으로 직접 등록 — 마감(16:00 ET) 5·10분 후, 개장(09:30 ET)
 * 10분 후. `cron` 패키지(v4, luxon 기반)가 이 타임존 안에서 서머타임(EDT/EST)을 자동 처리하므로
 * KST로 환산해 이중 등록할 필요도, 마감 시각이 다음날로 밀리는 요일 계산도 필요 없음 — 월요일
 * 마감은 그냥 월요일(1)로 잡으면 됨. (KST로만 등록하려면 서머타임 때마다 시각이 밀려서 EDT/EST
 * 두 벌을 수동으로 나눠야 했던 과거 버전을 이걸로 대체함.)
 *
 * env:
...
export class LaofusSchedulerService implements OnModuleInit {
  ...
  @Cron('5 16 * * 1-5', { name: 'laofus-reconcile-close-1', timeZone: 'America/New_York' })
  async reconcileClose1(): Promise<void> {
    await this.reconcileTick('마감+5분');
  }

  @Cron('10 16 * * 1-5', { name: 'laofus-reconcile-close-2', timeZone: 'America/New_York' })
  async reconcileClose2(): Promise<void> {
    await this.reconcileTick('마감+10분');
  }

  @Cron('40 9 * * 1-5', { name: 'laofus-reconcile-open', timeZone: 'America/New_York' })
  async reconcileOpen(): Promise<void> {
    await this.reconcileTick('개장+10분');
  }
