...
export class FilesController {
  ...
  @ApiOperation({
    ...
    description: '테스트용 단일 파일 업로드 엔드포인트입니다. 실제 업로드는 각 도메인 API에서 처리됩니다.',
  })
  ...
  @ApiOperation({
    ...
    description: '테스트용 다중 파일 업로드 엔드포인트입니다 (최대 5개). 실제 업로드는 각 도메인 API에서 처리됩니다.',
  })
  @ApiConsumes('multipart/form-data')
  @ApiResponse({
    status: 201,
  ...
  async uploadTestFiles(@UploadedFiles() files: Express.Multer.File[]) {
    ...
    const results = await Promise.all(files.map((file) => this.filesService.uploadImage(file, 'test')));

    return new SuccessBaseResponseDto('테스트 파일들을 업로드했습니다.', results);
  }
  ...
  @ApiOperation({
    summary: '예약용 사진 업로드',
    description: '예약 생성 전 사진을 사전 업로드합니다. 최대 5장까지 업로드 가능합니다.',
  })
  @ApiConsumes('multipart/form-data')
  @ApiResponse({
    status: 201,
  ...
  async uploadReservationPhotos(@UploadedFiles() files: Express.Multer.File[]) {
    if (!files || files.length === 0) {
      throw new BadRequestException(ERROR_MESSAGES.FILES.NOT_SELECTED);
    }
    if (files.length > 5) {
    ...
    const uploadResults = await Promise.all(files.map((file) => this.filesService.uploadImage(file, 'reservations')));
