...
import { useStrategyState, useTodayPlan } from '@/queries/iv.queries'
      ...
      <div className="card" style={{ marginBottom: 12 }}>
        ...
        {closePrice != null && (
          <div style={{ marginBottom: 12 }}>
            <span style={{ fontSize: 24, fontWeight: 800 }}>{fmtUSD(closePrice)}</span>
            <span style={{ marginLeft: 8, fontSize: 13, color: 'var(--color-text-secondary)' }}>종가</span>
          </div>
        )}
        ...
        {starPrice != null && (
          ...
          >
            <div>
              ...
              <div style={{ fontWeight: 700, color: 'var(--color-star)' }}>{fmtUSD(starPrice)}</div>
            </div>
            <div>
              <div style={{ fontSize: 11, color: 'var(--color-text-secondary)' }}>LOC 매수가</div>
              <div style={{ fontWeight: 700 }}>{fmtUSD(starPrice - 0.01)}</div>
            </div>
          </div>
        )}
              ...
              <tbody>
                {plan.buyRows.map((row, i) => {
                  ...
                  return (
                    <tr
                      key={i}
                      style={{ background: isStar ? 'var(--color-star-bg)' : 'transparent' }}
                    >
                      <td style={{ padding: '4px 0', fontWeight: isStar ? 700 : 400 }}>{row.label}</td>
                      <td style={{ textAlign: 'right', color: 'var(--color-rise)', fontWeight: 600 }}>
                        {fmtUSD(row.price)}
                      </td>
              ...
              <tbody>
                {plan.sellRows.filter((r) => (r.qty ?? 0) > 0).map((row, i) => (
                  <tr key={i}>
                    ...
                    <td style={{ textAlign: 'right', color: 'var(--color-fall)', fontWeight: 600 }}>
                      {fmtUSD(row.price)}
                    </td>
                    <td style={{ textAlign: 'right' }}>{row.qty != null ? `${row.qty}주` : '-'}</td>
                  </tr>
        ...
        {plan?.largeNumberBuy && (
          <div
            style={{
              padding: '8px 12px', background: '#fffbeb', border: '1px solid #fbbf24',
