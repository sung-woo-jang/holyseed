...
function StrategySummary({ id }: { id: string }) {
  ...
  return (
    ...
    >
      ...
      <div>
        <span style={{ color: 'var(--color-text-secondary)' }}>잔금 </span>
        <strong>{fmtUSD(state.cash)}</strong>
      </div>
      <span
        style={{
          fontSize: 10,
...
export function ExecCard({ exec, strategyId }: { exec: IvExecution; strategyId: string }) {
  ...
  const [execType, setExecType] = useState(exec.execType)
  ...
  const isNoExecEdit = execType === 'no_exec'
  ...
  async function handleSave() {
    if (isNoExecEdit) {
      await updateMut.mutateAsync({ execId: exec.id, execType })
    } else {
      const p = parseFloat(price)
      const q = parseFloat(qty)
      if (!p || !q) return
      await updateMut.mutateAsync({ execId: exec.id, execType, price: p, qty: q })
    }
    setEditing(false)
  }
  ...
  function cancelEdit() {
    setEditing(false)
    setExecType(exec.execType)
    setPrice(String(exec.execPrice ?? ''))
    setQty(String(exec.execQty ?? ''))
  }
    ...
    <div className="card" style={{ padding: '14px 16px', marginBottom: 10 }}>
      ...
      {editing ? (
        <>
          ...
          >
            <span>{EXEC_LABEL[execType] ?? execType}</span>
      ...
      {tBefore !== undefined && tAfter !== undefined && (
        <div style={{ fontSize: 13, color: 'var(--color-text-secondary)', marginBottom: 6 }}>
          ...
          {qtyAfter !== undefined && <span style={{ marginLeft: 8 }}>· {qtyAfter}주 보유</span>}
        </div>
      )}
...
function ExecutionList({ id }: { id: string }) {
  ...
  if (execs.length === 0) {
    return (
      <p style={{ color: 'var(--color-text-secondary)', textAlign: 'center', marginTop: 32 }}>체결 내역이 없습니다.</p>
    )
  }
...
export function HistoryPage() {
  ...
  return (
    <div style={{ padding: 16, paddingBottom: 80 }}>
      ...
      {strategies.length === 0 && (
        <p style={{ color: 'var(--color-text-secondary)', textAlign: 'center', marginTop: 40 }}>전략이 없습니다.</p>
      )}
    </div>
  )
}
