...
Holyseed 모노레포의 인프라 및 배포 관련 문서입니다. CI/CD, 서버 구성, 데이터베이스 마이그레이션 등을 관리합니다.
...
└─────────────────────────────────────────────────────┘
    ...
    (포트 8000)              (/var/www/holyseed-backoffice)
    PM2 managed             정적 파일 서빙
         ↓
┌─────────────────────────────────────────────────────┐
│ PostgreSQL (포트 5432)                               │
│ Docker Compose로 관리                               │
│ 데이터베이스: holyseed_prod                      │
└─────────────────────────────────────────────────────┘
```
...
- **위치**: `/etc/letsencrypt/live/holyseed.p-e.kr/`
- **파일**:
...
# ~/.ssh/config에 추가
cat >> ~/.ssh/config << 'EOF'
Host holyseed-external
    HostName 121.124.104.99
    User jangseong-u
    Port 2222
EOF

ssh holyseed-external
```
...
```bash
# psql 커맨드라인
psql -h 192.168.45.163 -p 5432 -U postgres -d holyseed
...
**CLI로 접속**:
```bash
psql -h 121.124.104.99 -p 5433 -U postgres -d holyseed
```
...
|-----|-------|------|------|-------|--------|
| 로컬/같은 네트워크 | 192.168.45.163 | 5432 | holyseed_prod_by_mac_mini | postgres | `Wkdtjddn062$` |
| 외부 네트워크 | 121.124.104.99 | 5433 | holyseed_prod_by_mac_mini | postgres | `Wkdtjddn062$` |
...
6. `pm2 restart holyseed-backend --update-env` - PM2 프로세스 재시작
...
#### Backoffice (holyseed-backoffice)
...
5. `rm -rf /var/www/holyseed-backoffice/*` - 기존 파일 제거
6. `cp -r dist/* /var/www/holyseed-backoffice/` - 빌드 결과 배포
7. HTTPS Health Check - `curl https://holyseed.p-e.kr` 확인

**소요 시간**: 약 1-2분

**배포 위치**: `/var/www/holyseed-backoffice/`
...
4. Health Check - `curl https://holyseed.p-e.kr/health` 확인
...
```
holyseed-backend/actions-runner/
holyseed-backoffice/actions-runner/
```
...
```bash
# Backend Runner 시작
cd ~/Desktop/project/holyseed/holyseed-backend/actions-runner
./run.sh

# Backoffice Runner 시작
cd ~/Desktop/project/holyseed/holyseed-backoffice/actions-runner
./run.sh
```
...
cd ~/Desktop/project/holyseed/holyseed-backend/actions-runner && ./run.sh &
cd ~/Desktop/project/holyseed/holyseed-backoffice/actions-runner && ./run.sh &

# 또는 백그라운드에서 실행
nohup ~/Desktop/project/holyseed/holyseed-backend/actions-runner/run.sh &
nohup ~/Desktop/project/holyseed/holyseed-backoffice/actions-runner/run.sh &
```
...
- **원본 파일**: `holyseed/infra/nginx/holyseed.conf`
- **심볼릭 링크**: `/usr/local/etc/nginx/conf.d/holyseed.conf` → `infra/nginx/holyseed.conf`

### 설정 파일 수정 방법

1. 로컬에서 `infra/nginx/holyseed.conf` 수정
...
```nginx
ssl_certificate /etc/letsencrypt/live/holyseed.p-e.kr/fullchain.pem;
ssl_certificate_key /etc/letsencrypt/live/holyseed.p-e.kr/privkey.pem;
```
...
# 3. 배포 완료 후 https://holyseed.p-e.kr 확인
```
...
```bash
cd ~/Desktop/project/holyseed/holyseed-backend
...
# PM2 재시작
pm2 restart holyseed-backend --update-env
pm2 save
...
```bash
cd ~/Desktop/project/holyseed/holyseed-backoffice
...
# 배포
rm -rf /var/www/holyseed-backoffice/*
cp -r dist/* /var/www/holyseed-backoffice/

# 검증
curl https://holyseed.p-e.kr
```
...
```bash
# PM2 로그 (실시간)
pm2 logs holyseed-backend

# PM2 로그 (마지막 100줄)
pm2 logs holyseed-backend --lines 100

# PM2 프로세스 상태
pm2 status

# PM2 프로세스 상세 정보
pm2 show holyseed-backend
```
...
```bash
# Access 로그 (실시간)
tail -f /usr/local/var/log/nginx/holyseed-access.log

# Error 로그
tail -f /usr/local/var/log/nginx/holyseed-error.log

# Access 로그 (마지막 50줄)
tail -n 50 /usr/local/var/log/nginx/holyseed-access.log

# 404 에러만 필터링
grep " 404 " /usr/local/var/log/nginx/holyseed-access.log
```
...
```bash
# Docker 컨테이너 로그 (실시간)
docker logs -f holyseed_postgres

# Docker 컨테이너 로그 (마지막 100줄)
docker logs holyseed_postgres --tail 100

# 컨테이너 상태 확인
docker ps | grep holyseed_postgres
```
...
- **Backend Deploy**: holyseed-backend 변경 시 실행
- **Backoffice Deploy**: holyseed-backoffice 변경 시 실행
...
# 2. 에러가 있으면 파일 확인
cat /usr/local/etc/nginx/conf.d/holyseed.conf

# 3. Nginx 재로드 (무중단)
nginx -s reload

# 4. 상태 확인
curl https://holyseed.p-e.kr/health
```
...
# 2. 컨테이너 시작 여부 확인
docker exec -it holyseed_postgres pg_isready -U postgres

# 3. 컨테이너 로그 확인
docker logs holyseed_postgres

# 4. 필요하면 컨테이너 재시작
docker restart holyseed_postgres
...
| `holyseed_user` | `.env.production`의 `DB_PASSWORD` 값 | 제한된 권한 | 애플리케이션 연동 (현재는 미사용) |

**사용자 생성 (필요 시)**:

```bash
# holyseed_user 계정 생성
docker exec holyseed_postgres psql -U postgres -c \
  "CREATE USER holyseed_user WITH PASSWORD 'Holyseed2024!SecureDB#Pass';"

# 데이터베이스 권한 부여
docker exec holyseed_postgres psql -U postgres -c \
  "GRANT ALL PRIVILEGES ON DATABASE holyseed TO holyseed_user;"

# 스키마 권한 부여
docker exec holyseed_postgres psql -U postgres -d holyseed -c \
  "GRANT ALL ON SCHEMA public TO holyseed_user;"
```
...
# 인증서 자세히 보기
openssl x509 -in /etc/letsencrypt/live/holyseed.p-e.kr/fullchain.pem -text -noout | grep -A 2 "Validity"

# HTTPS 연결 확인
openssl s_client -connect holyseed.p-e.kr:443
```
...
- **전체 프로젝트 가이드**: `holyseed/CLAUDE.md`
- **Backend 가이드**: `holyseed-backend/CLAUDE.md`
- **Backoffice 가이드**: `holyseed-backoffice/CLAUDE.md`
- **API 명세**: `holyseed-front/docs/api/API_SPECIFICATION.md`
...
| `/var/www/holyseed-backoffice` | Backoffice 빌드 결과 |
| `/usr/local/etc/nginx/conf.d/` | Nginx 설정 디렉토리 |
| `/etc/letsencrypt/live/holyseed.p-e.kr/` | SSL 인증서 |
