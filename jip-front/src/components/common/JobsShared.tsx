...
export const JIcon = {
  ...
  Check: ({ s = 12 }: { s?: number }) => (
    <svg width={s} height={s} viewBox="0 0 14 14" fill="none" stroke="currentColor" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round">
  ...
  if (fileUrl) {
    return (
      <div className={`jobs-photo ${role}`} style={style}>
        <div className="corner">{role === 'before' ? 'BEFORE' : 'AFTER'}{idx != null ? ` ${String(idx + 1).padStart(2, '0')}` : ''}</div>
        <img src={fileUrl} alt={displayLabel} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
      </div>
    )
  }
  return (
...
interface JStatusPillProps { status?: string }
...
interface JPubToggleProps { on: boolean; onClick: () => void; title?: string }
export function JPubToggle({ on, onClick, title }: JPubToggleProps) {
  return (
    <button type="button" className={`jobs-pub ${on ? 'on' : ''}`} onClick={onClick} title={title ?? (on ? '공개 중' : '숨김')}>
      <span className="box">
        <svg width="10" height="10" viewBox="0 0 14 14" fill="none">
          <path d="M2.5 7.4L5.6 10.5 11.5 3.5" stroke="white" strokeWidth="2.4" strokeLinecap="round" strokeLinejoin="round" />
        </svg>
      </span>
      공개
    </button>
