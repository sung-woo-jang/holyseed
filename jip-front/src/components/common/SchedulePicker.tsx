...
function isPast(d: Date, today: Date) { return d < today }
  ...
  return (
    <div className="sched-picker">
      ...
      <div className="sched-head">
        <div className="sched-month">{cursor.getFullYear()}년 {cursor.getMonth() + 1}월</div>
        <div className="sched-nav">
          <button type="button" className="sched-nav-btn" disabled={!canGoPrev}
            onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() - 1, 1))}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
              <polyline points="15 18 9 12 15 6" />
            </svg>
          </button>
          <button type="button" className="sched-nav-btn" disabled={!canGoNext}
            onClick={() => setCursor(new Date(cursor.getFullYear(), cursor.getMonth() + 1, 1))}>
            <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
      ...
      <div className="sched-legend">
        <span className="sched-legend-item"><span className="sched-dot open" />가능</span>
        <span className="sched-legend-item"><span className="sched-dot busy" />일부 가능</span>
        <span className="sched-legend-item"><span className="sched-dot full" />마감</span>
        <span className="sched-legend-item"><span className="sched-dot off" />휴무</span>
      </div>

      {/* 요일 헤더 */}
      <div className="sched-grid sched-weekdays">
        {WEEKDAYS.map((w, i) => (
          <div key={w} className={`sched-weekday${i === 0 ? ' sun' : i === 6 ? ' sat' : ''}`}>{w}</div>
        ))}
      </div>
        ...
        {cells.map((d, i) => {
          ...
          return (
            ...
            >
              ...
              {!disabled && status === 'off' && (
                <span className="sched-cell-meta off-label">휴무</span>
              )}
            </button>
          )
        })}
      ...
      {value.date && selectedDateObj && (
        <div className="sched-slots">
          <div className="sched-slots-head">
            ...
            <div className="muted" style={{ fontSize: 13 }}>가능한 가장 이른 시간</div>
          </div>
