  ...
  return (
    <>
      <div className="spread mb-24" style={{ alignItems: 'end', flexWrap: 'wrap' }}>
        ...
        <span className="mono muted" style={{ fontSize: 13 }}>{new Date().toLocaleDateString('ko-KR')}</span>
      </div>
      ...
      <div className="admin-grid">
        ...
        <div className="stat-card">
          ...
          <div className="delta" style={{ color: 'var(--orange)' }}>일정 잡힌 작업</div>
        </div>
        <div className="stat-card">
          <div className="lbl">이번 달 매출</div>
          <div className="num" style={{ fontSize: 26 }}>{data ? fmtKRW(data.monthlyRevenue) : '—'}</div>
          ...
          <tbody>
            {data?.recentRequests.map((r) => (
              <tr key={r.id}>
                ...
                <td><span className={`tag${r.status === 'done' ? ' green' : r.status === 'pending' ? ' orange' : ''}`}>{STATUS_LABEL[r.status] ?? r.status}</span></td>
            ...
            {(!data || data.recentRequests.length === 0) && (
              <tr><td colSpan={6} style={{ textAlign: 'center', padding: 40, color: 'var(--ink-3)' }}>요청이 아직 없어요.</td></tr>
            )}
          </tbody>
        </table>
      </div>

      <div className="mt-32 spread" style={{ gap: 12 }}>
        <button className="btn ghost" onClick={() => navigate('/admin/requests')}>전체 요청 보기 →</button>
        <button className="btn ghost" onClick={() => navigate('/admin/jobs')}>시공 일지 →</button>
      </div>
    </>
  )
}
