...
export default function AdminRequestDetail() {
  ...
  const saveSchedule = async () => {
    ...
    try {
      ...
      setR((prev) =>
        prev
          ? { ...prev, prefDate: sched.date || null, prefTimeSlot: sched.time != null ? `${sched.time}시 이후` : null }
          : prev
      )
      showToast('일정이 저장됐어요')
    } catch {
  ...
  return (
    <>
      <div className="steps mb-16">
        <span className="link" onClick={() => navigate('/admin/requests')}>
          견적 요청
        </span>
            ...
            <div className="card card-pad">
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 10 }}>
                {r.photos!.map((p, i) => (
                  <div
                    key={i}
                    style={{ aspectRatio: '1', borderRadius: 8, overflow: 'hidden', background: 'var(--bg)' }}
                  >
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 24 }}>
          <div className="card card-pad">
            ...
            <dl style={{ display: 'grid', gridTemplateColumns: '72px 1fr', gap: '8px 12px', fontSize: 14, margin: 0 }}>
              ...
              <dd className="mono">{r.contactPhone}</dd>
              <dt className="muted">주소</dt>
              <dd>{r.contactAddress}</dd>
              <dt className="muted">희망일</dt>
              <dd>
                {r.prefDate ?? '—'} {r.prefTimeSlot ?? ''}
              </dd>
              <dt className="muted">접수</dt>
              <dd className="mono">{new Date(r.createdAt).toLocaleDateString('ko-KR')}</dd>
            </dl>
            <a className="btn ink mt-16 block" href={`tel:${r.contactPhone}`}>
              {r.contactPhone} 전화
            </a>
          </div>
          ...
          <div className="card card-pad">
            ...
            <button className="btn primary mt-16 block" onClick={saveSchedule} disabled={savingSched}>
          ...
          <div className="card card-pad">
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {action && (
                <button className="btn primary block" onClick={() => updateStatus(action.next)}>
                  {action.label}
                </button>
              )}
              {r.status === 'pending' && (
                <button className="btn ghost block" onClick={() => updateStatus('cancelled')}>
                  요청 거절
                </button>
              )}
