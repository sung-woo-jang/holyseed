        ...
        <div className="store-page mt-64">
          ...
          <section className="store-block store-tagline">
            ...
            <h2 className="store-tagline-text">
              {cat?.name === '주방'
                ? '매일 쓰는 주방, 작은 것부터 바꿔봐요.'
                : cat?.name === '욕실'
                  ? '하루의 시작과 끝, 욕실이 달라집니다.'
                  : cat?.name === '필름'
                    ? '필름 한 장으로 집 분위기가 바뀝니다.'
                    : cat?.name === '바닥'
                      ? '발 끝의 감촉부터 다시.'
                      : '집의 작은 디테일.'}
            </h2>
                ...
                {product.features.slice(0, 3).map((f, i) => (
                  <div key={f.id} className="store-reason">
                    ...
                    <div className="store-reason-desc">
                      {i === 0
                        ? '제품 본체부터 신경 썼어요. 오래 써도 변형이 적습니다.'
                        : i === 1
                          ? '매일 쓰는 곳이라 잔고장이 없어야 해요. 검증된 부품만 사용해요.'
          ...
          <section className="store-block store-compare-block">
            ...
            <div className="store-compare">
              ...
              <div className="store-compare-side after">
                ...
                <div className="store-compare-caption">
                  {product.brand} {product.name}으로 깔끔하게 교체
                </div>
              </div>
            </div>
          </section>
              ...
              {[
                {
                  ...
                  name: '박○○ 고객',
                  stars: 5,
                  txt: '오래된 제품을 교체했는데 분위기가 확 바뀌었어요. 깔끔하게 시공해주셔서 만족합니다.',
                },
                {
                  area: '강남구 역삼동',
                  name: '이○○ 고객',
                  stars: 5,
                  txt: '아침에 연락드렸는데 그날 저녁에 바로 와주셨어요. 마무리도 깨끗하게 해주셨습니다.',
                },
                {
                  area: '마포구 망원동',
                  name: '김○○ 고객',
                  stars: 4,
                  txt: '가격도 합리적이고 친절하셨어요. 다음에 다른 시공도 부탁드리려구요.',
                },
              ].map((r, i) => (
                <div key={i} className="store-review">
                  <div className="store-review-stars">
                    {Array.from({ length: 5 }).map((_, j) => (
                      <span key={j} className={j < r.stars ? 'on' : ''}>
                        ★
                      </span>
                    ))}
                  </div>
          ...
          <div className="product-section">
            ...
            <div className="faq-list mt-32">
              {[
                {
                  q: '제품 색상은 어디서 확인하나요?',
                  a: '시공 당일 색상 샘플을 가져가서 함께 보고 결정합니다. 사진과 실물이 다를 수 있어서 꼭 현장에서 확인해요.',
                },
                {
                  q: 'A/S는 어떻게 받나요?',
                  a: '제품 자체 보증과 별도로, 시공 후 6개월 동안은 시공 관련 무료 A/S를 제공합니다. 그 이후엔 출장비만 받고 점검 드려요.',
                },
                {
                  q: '결제는 언제 하나요?',
                  a: '시공이 완료된 뒤에 정산합니다. 방문비 2만원은 예약 시 선결제하지만, 시공으로 이어지면 총액에 포함돼서 차감돼요.',
                },
                {
                  q: '이 제품 말고 다른 제품도 가능한가요?',
                  a: '네, 카탈로그에 없는 제품도 시공 가능합니다. 견적 요청 시 메모에 원하시는 제품을 적어주세요.',
                },
              ].map((f, i) => (
                <FAQItem key={i} {...f} />
              ))}
