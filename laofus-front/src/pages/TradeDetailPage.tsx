...
import type { TossOrderDto } from '@/lib/types'
import { api, kst, kstDateOnly, n, usd } from '@/lib/types'
    ...
    return (
      <main className="wrap">
        <p style={{ color: 'var(--text-muted)' }}>
          거래 없음 — <Link to="/cycles">사이클 목록</Link>
        </p>
      </main>
    )
  ...
  return (
    <main className="wrap">
      <div style={{ display: 'flex', gap: 12, alignItems: 'baseline', marginBottom: 12, flexWrap: 'wrap' }}>
        <Link to={`/cycles/${cycle.cycleNo}`} style={{ fontSize: 13, color: 'var(--text-secondary)' }}>
          ← {cycle.cycleNo}차 사이클
        </Link>
