...
export async function createIcons() {
  ...
  const frontendDocsPath = path.resolve(__dirname, '../../../..', 'living-craft-front/docs')
  ...
  console.log(`   - FILL: ${parsedIcons.filter((i) => i.type === 'FILL').length}개`)
  console.log(`   - MONO: ${parsedIcons.filter((i) => i.type === 'MONO').length}개`)
  console.log(`   - COLOR: ${parsedIcons.filter((i) => i.type === 'COLOR').length}개\n`)
}
