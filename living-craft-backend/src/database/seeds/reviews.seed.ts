...
export async function createReviews(): Promise<void> {
  ...
  if (completedReservations.length === 0) {
    console.log('⚠️  No completed reservations found. Please run reservations seed first.')
    return
  }

  // 30건 또는 완료된 예약의 60% 중 작은 값
  const reviewCount = Math.min(30, Math.floor(completedReservations.length * 0.6))

  // 랜덤하게 선택 (shuffle로 중복 방지)
  const selectedReservations = faker.helpers.shuffle(completedReservations).slice(0, reviewCount)
  ...
  const reviewTemplates: Record<number, string[]> = {
    ...
    2: ['기대에 못 미쳤어요. 다시 보수 작업이 필요할 것 같습니다.', '시공 품질이 기대 이하였습니다.'],
