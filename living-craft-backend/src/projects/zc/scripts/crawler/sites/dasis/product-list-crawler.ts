  ...
  private async extractProductsFromPage(page: Page, categoryId: string): Promise<ProductBasic[]> {
    ...
    $('.item_basket_type li .item_cont').each((index, element) => {
      ...
      try {
        ...
        // 모델명 추출 - .num_model span에서 직접 가져오기
        let modelName: string | undefined = undefined
        const modelSpan = $el.find('.num_model').text().trim()
        if (modelSpan) {
          // "모델번호 : ALB-R14600" 형태에서 모델번호만 추출
          modelName = modelSpan.replace(/모델번호\s*:\s*/g, '').trim()
        }

        // .num_model이 없으면 제품명에서 추출 (백업)
        if (!modelName) {
          modelName = extractModelName(name)
        }
    ...
    // 가격이 0인 제품이 있으면 세션 만료로 간주
    const zeroPriceProducts = products.filter(p => p.price === 0)
    if (zeroPriceProducts.length > 0) {
      logger.error(`가격이 0인 제품 ${zeroPriceProducts.length}개 발견. 세션이 만료되었을 수 있습니다.`)
      throw new Error('세션이 만료되었습니다. 로그인이 필요합니다. GD5SESSID 쿠키를 갱신하세요.')
    }
