/**
 * 미국 시장의 현재 활성 세션(PRE/REGULAR/AFTER) 판별 — 순수함수 (API 클라이언트 비의존).
 *
 * laofus의 checkWindow()와 달리 VR은 "마감까지 몇 분"이 아니라 "지금 어떤 세션이 열려있는가"만
 * 필요하다. 토스 market-calendar/US가 previousBusinessDay/today/nextBusinessDay 3영업일을
 * 주므로 전부 검사한다(자정 근처 KST 날짜 경계 문제 회피 — laofus checkWindow와 동일한 이유).
 */
